    ...
    if (_displayCountingLabel)
    {
        ...
        switch (_chartType) {
            case NKChartFormatTypePercent:
                format = @"%d%%";
                break;
            case NKChartFormatTypeCal:
                format = @"%d千卡";
                break;
            case NKChartFormatTypeWeight:
            default:
                format = @"%dkg";
                break;
        }
        ...
        CGRect textRect = CGRectMake(arcCenter.x-30.0, arcCenter.y+5.0, 60.f, 30.f);
        ...
        [text drawInRect:textRect
           withAttributes:@{ NSParagraphStyleAttributeName:priceParagraphStyle, NSFontAttributeName:[UIFont boldSystemFontOfSize:20], NSForegroundColorAttributeName:self.strokeColor}];
        
    }
    [self drawTextInContext:UIGraphicsGetCurrentContext() text:self.lblTitle inRect:CGRectMake(arcCenter.x-50.0, arcCenter.y-10.0, 100.f, 20.f) font:[UIFont systemFontOfSize:8]];
    
    
    UIImage* im = [UIImage imageNamed:self.imgName];
    [im drawInRect:CGRectMake(arcCenter.x-7.5, arcCenter.y-30.0, 15, 14)];
    
    UIImage* combinedShape = [UIImage imageNamed:@"Combined Shape"];
    [combinedShape drawInRect:CGRectMake(arcCenter.x-3.5,_frame.size.width/2-radius+1, 7, 6.5)];
...
- (void)drawTextInContext:(CGContextRef )ctx text:(NSString *)text inRect:(CGRect)rect font:(UIFont *)font
{
    ...
    [text drawInRect:rect
      withAttributes:@{ NSParagraphStyleAttributeName:priceParagraphStyle, NSFontAttributeName:font, NSForegroundColorAttributeName:[UIColor colorWithRed:158/255.0 green:158/255.0 blue:158/255.0 alpha:1.0]}];
}
