...
- (void)tap:(UIButton *)button{
    ...
    if (self.detailView.superview) {
        [UIView animateWithDuration:.15 animations:^{
            ...
            if(currentTag>=0)
                [self displayDetailViewAtPoint:CGPointMake(button.center.x, button.center.y-30)];
        }];
        
    }else [self displayDetailViewAtPoint:CGPointMake(button.center.x, button.center.y-30)];
...
-(UIView <MPDetailView> *)detailView{
    ...
    label.textColor=[UIColor colorWithRed:163/255.0 green:163/255.0 blue:163/255.0 alpha:1];
    label.font=[UIFont fontWithName:@"Haettenschweiler" size:25];
